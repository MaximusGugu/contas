...
async function salvarFirebase() {
  ...
  try {
    ...
    // CORREÇÃO: caixinhas deve estar dentro das chaves { } junto com os outros dados
    const pacote = await encryptData({ 
        ...
        cartoes, 
        gastosDetalhes,
        caixinhas // Agora está no lugar certo
    }, senhaDoUsuario);
...
function atualizarTudo(anoParaVisualizar, pendente = true) {
  ...
  anosOrdenados.forEach(ano => {
    ...
    dados[ano].meses.forEach((m, idx) => {
      // Inicialização de controles de desativação se não existirem
      if (!m.fixasDesativadas) m.fixasDesativadas = {};
      if (!m.receitasDesativadas) m.receitasDesativadas = {};

      // Lógica de cascata do saldo em conta
      ...
      // --- CÁLCULO DE RENDAS (Ajustado aqui) ---
      
      // 1. Soma das Rendas Recorrentes (Rendas Fixas)
      const totalRendasFixas = receitasFixas
        .filter(rf => rf.ativo && !m.receitasDesativadas[rf.id])
        .reduce((acc, rf) => acc + rf.valor, 0);

      // 2. Soma das Rendas Manuais (Outros/Empresa)
      const eTotal = (m.empresa || []).filter(x => x.checked).reduce((acc, b) => acc + b.valor, 0);

      // 3. Total Disponível (Salário Base + Saldo em Conta + Rendas Manuais + Rendas Recorrentes)
      const tDisp = (m.salario || 0) + (m.conta || 0) + eTotal + totalRendasFixas;
                  ...
                  Object.keys(totaisPorCartao).forEach(cid => {
                      const cObj = cartoes.find(c => c.id == cid);
                        if (cObj) {
                            ...
                            itemC.onmouseenter = () => {
                                itemC.style.backgroundColor = corCard + "33"; 
                                itemC.style.transform = "translateX(5px)";
                            };
                            itemC.onmouseleave = () => {
                                itemC.style.backgroundColor = ""; 
...
function criarItem(lista, d, dataArray, ano) {
  ...
  btn.onclick = () => {
        if (d.caixinhaId) {
        if (!confirm("Este item está vinculado a uma Caixinha. Removê-lo alterará o saldo da sua reserva. Deseja continuar?")) return;
        }
      if(d.parcelaId) {
            ...
            <div class="conteudoColuna">
                ...
                <div class="area-acoes-despesa" style="margin-top:15px; padding-top:15px; border-top:1px dashed rgba(255,255,255,0.1);">
                    <div style="display:flex; gap:8px; margin-bottom:10px;">
                        <button class="btn btn-show-quick-add" style="flex:1; font-size:11px;">+ DESPESA RÁPIDA</button>
                        <button class="btn btn-show-quick-parcela" style="flex:1; font-size:11px;">+ PARCELAMENTO</button>
                    </div>

                    <!-- FORMULÁRIO DESPESA RÁPIDA -->
                    <div class="form-rapido-despesa" style="display:none; flex-direction:column; gap:8px; background:rgba(0,0,0,0.2); padding:10px; border-radius:8px;">
                        <input type="text" class="inputPadrao q-nome" placeholder="Nome da despesa">
...
// --- LÓGICA DE EVENTOS DO FORMULÁRIO RÁPIDO (CORRIGIDO) ---
  const btnShowAdd = body.querySelector(".btn-show-quick-add");
  const btnShowParcela = body.querySelector(".btn-show-quick-parcela");
  ...
  const buttonGroup = btnShowAdd.parentElement; // O div que contém os dois botões iniciais

  btnShowAdd.onclick = () => { 
      formQuick.style.display = "flex"; 
      buttonGroup.style.display = "none"; // Esconde os botões "+ Despesa" e "+ Parcela"
  };

  btnCancelQuick.onclick = () => { 
      formQuick.style.display = "none"; 
      buttonGroup.style.display = "flex"; // Mostra os botões de volta
  };

  btnConfirmQuick.onclick = () => {
      const nomeInput = formQuick.querySelector(".q-nome");
      const valorInput = formQuick.querySelector(".q-valor");
      const nome = nomeInput.value.trim();
      const valor = parseValor(valorInput.value);

      if(nome && valor > 0) {
          data.despesas.push({ nome, valor, checked: true });
          nomeInput.value = "";
          valorInput.value = "";
          formQuick.style.display = "none";
  ...
  btnShowParcela.onclick = () => window.abrirModalParcelamento(index, ano);

  // --- RENDEREZAÇÃO DOS ITENS ---
  const listD = body.querySelector(".listaDesp"); 
  const listE = body.querySelector(".listaEmp");

  // Snapshots de Despesas Fixas
  ...
  // Configuração dos inputs de salário e saldo
  const inS = body.querySelector("input.salario");
  const inC = body.querySelector("input.conta");
  
  inS.value = formatar(data.salario);
  inC.value = formatar(data.conta);

  aplicarComportamentoInput(inS, () => data.salario, (v) => { data.salario = v; atualizarTudo(ano); }, ano);
  aplicarComportamentoInput(inC, () => data.conta, (v) => { data.conta = v; data.contaManual = true; atualizarTudo(ano); }, ano);

  body.querySelector(".addEmp").onclick = () => {
      data.empresa.push({ nome: "", valor: 0, checked: true });
      carregarAno();
  };
    // --- LÓGICA DO BOTÃO CASCATA (🔗) ---
  const btnCascata = body.querySelector(".btn-cascata");
  btnCascata.onclick = () => {
      // 1. Pega todos os anos cadastrados e coloca em ordem
      const anos = Object.keys(dados).map(Number).sort((a, b) => a - b);
      let encontrouMesAtual = false;

      // 2. Percorre todos os anos e meses
      anos.forEach(a => {
          dados[a].meses.forEach((m, i) => {
              // Se chegarmos no mês onde o botão foi clicado
              if (a == ano && i == index) {
                  encontrouMesAtual = true;
              } 
              // Para todos os meses que vêm DEPOIS do atual
              else if (encontrouMesAtual) {
                  m.contaManual = false; // Remove a "trava" manual do campo conta
              }
          });
      });

      // 3. Recalcula tudo para os valores fluírem
      atualizarTudo(ano);
      carregarAno(); // Atualiza a visualização para mostrar as bordas verdes (automático)
      alert("Cascata ativada! Os meses seguintes agora seguem o saldo automaticamente.");
  };
    ...
    for (let m = 0; m < 12; m++) {
        ...
        mesBox.innerHTML = `
            ...
            <div class="mesBody">
                ...
                <div class="resumo-gastos-inferior" style="margin-top: 10px; display: flex; flex-direction: column; gap: 5px;">
                    <div class="barra-resumo credito"><span>Crédito (Variável + Fixo):</span> <span>${formatar(tCr)}</span></div>
                    <div class="barra-resumo debito"><span>Débito (Variável + Fixo):</span> <span>${formatar(tDb)}</span></div>
        ...
        // --- 1. RENDERIZAR DESPESAS FIXAS (ASSINATURAS NO CARTÃO) ---
        if (gastosFixos.length > 0) {
            const trHeader = document.createElement("tr");
            trHeader.innerHTML = `<td colspan="6" style="background: rgba(255,255,255,0.05); font-size: 10px; font-weight: bold; color: var(--P04); padding: 5px 12px; letter-spacing: 1px;">DESPESAS FIXAS SINCRONIZADAS</td>`;
            ...
            gastosFixos.forEach(g => {
                ...
                const cardCor = cartoes.find(c => c.id == g.cartaoId)?.color || "transparent";
                
                tr.style.opacity = desativada ? "0.3" : "1";
                tr.innerHTML = `
                    <td><input type="checkbox" ${!desativada ? 'checked' : ''}></td>
                    <td style="font-style: italic;">${g.nome} <small>(fixo)</small></td>
                    <td><span class="badge" style="border: 1px solid ${catCor}; color: ${catCor}">${g.categoria}</span></td>
                    <td style="color: ${cardCor}; font-weight: bold;">💳 ${cartoes.find(c => c.id == g.cartaoId)?.nome}</td>
            ...
            trSpacer.innerHTML = `<td colspan="6" style="background: rgba(255,255,255,0.05); font-size: 10px; font-weight: bold; color: var(--P02); padding: 5px 12px; letter-spacing: 1px;">GASTOS VARIÁVEIS</td>`;
            tbody.appendChild(trSpacer);
        }

        // --- 2. RENDERIZAR GASTOS VARIÁVEIS (MANUAIS) ---
        gastosManuais.forEach(g => {
            ...
            inNome.onblur = () => { g.nome = inNome.value; salvarDadosLocal(); };
            
            selCat.onchange = (e) => { 
                g.categoria = e.target.value; 
                const novaCor = categorias.find(c => c.name === e.target.value)?.color || "transparent";
                selCat.style.borderLeft = `5px solid ${novaCor}`;
                salvarDadosLocal(); 
            ...
            selCard.onchange = (e) => { 
                ...
                const novaCor = cartoes.find(c => String(c.id) === String(e.target.value))?.color || "transparent";
                selCard.style.borderLeft = `5px solid ${novaCor}`;
                salvarDadosLocal(); 
                renderPaginaGastos(); // Recarrega para atualizar totais do mês
                atualizarTudo(anoView); 
            };

            aplicarComportamentoInput(inVal, () => g.valor, (v) => { g.valor = v; atualizarTudo(anoView); renderPaginaGastos(); }, anoView);
            ...
            tr.querySelector(".removeItem").onclick = () => {
                ...
                salvarDadosLocal(); carregarAno(); renderPaginaGastos();
            };
            tbody.appendChild(tr);
        });

        // Eventos de clique do rodapé e controles
        ...
        mesBox.querySelector(`#btn-add-${m}`).onclick = () => {
        ...
        mesBox.querySelector(`#btn-add-parcela-${m}`).onclick = () => {
            window.abrirModalParcelamento(m, anoView);
        };
...
document.getElementById("btnSalvarConfig").onclick = async () => { 
    // Lê os novos valores dos inputs
    ...
    // Atualiza a interface para refletir as mudanças (como o dia de virada)
    carregarAno(); 
    renderPaginaGastos();
};
...
document.getElementById("btnAddContaFixa").onclick = () => { 
    contasFixas.push({ 
        id: "f" + Date.now(), // ID único como string
        nome: "Nova Despesa", 
        valor: 0, 
        dia: 1,
        ativo: true, 
...
document.getElementById("btnAddAssinaturaFixa").onclick = () => { 
    contasFixas.push({ 
        id: "a" + Date.now(), // ID único como string
                        ...
                        ${info.historico.length > 0 ? info.historico.map(h => `
                            <div class="cx-history-item ${h.tipo}">
                                <div>
                                    ...
                                    <strong style="font-size: 12px;">${h.tipo === 'resgate' ? 'SAQUE' : h.tipo === 'rendimento' ? 'RENDIM.' : 'DEPÓSITO'}</strong>
                                </div>
...
function obterDadosCaixinha(id) {
    ...
    anos.forEach(ano => {
        if (!dados[ano].meses) return;
        dados[ano].meses.forEach((m, idx) => {
            // A) Vindo do botão "Guardar" (está em despesas)
            ...
            // B) Vindo do botão "Resgatar" (está em empresa/rendas)
            (m.empresa || []).forEach(r => {
            ...
            // C) Vindo do botão "Depósito" direto no card (está em movimentosCaixinha)
    ...
    // Ordem de lançamento (Lançado por último fica embaixo)
...
window.excluirMovimentoCaixinha = (ano, mesIdx, origem, timestamp) => {
